...
【省力化のポイント】
本設備の導入により最も大きな効果が得られるのは、これまで手作業で行っていた工程の自動化である。{self.e.name}のAI機能により、これらの作業が自動化され、従業員は判断や顧客対応といった人間にしかできない付加価値の高い業務に集中できるようになる。

1日あたりの削減時間は{self.l.reduction_hours:.1f}時間となり、月間では約{self.l.reduction_hours * Config.WORKING_DAYS_PER_MONTH:.0f}時間の業務時間を創出できる。"""
...
def generate_diagrams(data: HearingData, output_dir: str) -> Dict[str, str]:
    ...
    specs = [
        ...
        ("04_課題フロー", f"課題の連鎖図\n人手不足→業務過多→残業増加({s.overtime_hours}h)→離職リスク→さらなる人手不足"),
        ("05_設備概要", f"導入設備概要\n名称:{e.name}\n金額:{e.total_price:,}円\n特徴:AI活用、自動化"),
        ("06_ビフォーアフター", f"ビフォーアフター比較図\n導入前:{l.current_hours}時間\n導入後:{l.target_hours}時間\n削減:{l.reduction_hours:.1f}時間\n削減率:{l.reduction_rate:.0f}%"),
...
def generate_business_plan_1_2(data: HearingData, diagrams: Dict[str, str], output_dir: str, template_path: Path):
    ...
    if len(doc.tables) > 4:
        ...
        if diagrams:
            print("    🖼️ 図解挿入...")
            mapping = {1: ["01_企業概要", "02_SWOT分析"], 2: ["03_人手不足", "04_課題フロー"],
                       4: ["05_設備概要"], 5: ["06_ビフォーアフター"], 6: ["07_効果算定"],
